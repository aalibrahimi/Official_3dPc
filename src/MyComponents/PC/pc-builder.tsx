  ...
  useFrame((state) => {
    if (group.current) {
      ...
      group.current.rotation.y =
        Math.sin(state.clock.getElapsedTime() * 0.1) * 0.05 + Math.PI / 6;
    }
  });
...
function ComponentSlot({
  ...
  const [hoveblue, setHoveblue] = useState(false);
  const part = selectedParts[type];
  const isAnimating =
    activeAnimation.partType === type && activeAnimation.isAnimating;
    ...
    if (meshRef.current) {
      if (hoveblue) {
        ...
        meshRef.current.scale.x =
          1 + Math.sin(state.clock.getElapsedTime() * 5) * 0.05;
        meshRef.current.scale.y =
          1 + Math.sin(state.clock.getElapsedTime() * 5) * 0.05;
        meshRef.current.scale.z =
          1 + Math.sin(state.clock.getElapsedTime() * 5) * 0.05;
      } else {
        // Smoothly return to normal size when not hoveblue
  ...
  return (
    <group position={position}>
      ...
      <mesh
        ref={meshRef}
        onPointerOver={() => setHoveblue(true)}
        onPointerOut={() => setHoveblue(false)}
      >
        <boxGeometry args={[1, 0.5, 0.2]} />
        <meshStandardMaterial
          color={part ? "#5f27cd" : "#333333"}
          emissive={part ? "#5f27cd" : hoveblue ? "#444444" : "#000000"}
          emissiveIntensity={part ? 0.5 : hoveblue ? 0.3 : 0}
          metalness={0.8}
          roughness={0.2}
        />
      </mesh>
      ...
      <Html position={[0, -0.4, 0]} center>
        <div className="text-xs font-medium text-white bg-black/50 px-2 py-1 rounded-full">
          {name}
        </div>
      </Html>
    ...
    <mesh
      ...
      position={
        isAnimating
          ...
          : [slotPosition[0], slotPosition[1], slotPosition[2] + 0.2]
      }
    >
      {/* Simple box geometry to represent the part */}
      <boxGeometry args={[0.8, 0.4, 0.1]} />
      <meshStandardMaterial
        color={part.color || "#5f27cd"}
        metalness={0.9}
        roughness={0.1}
      />
  ...
  return (
    <group ref={linesRef}>
      ...
      {Array.from({ length: 15 }).map((_, i) => (
        <mesh
          key={i}
          position={[-0.5 + Math.random() * 1, -1.5 + Math.random() * 3, -0.4]}
        >
          ...
          <meshBasicMaterial
            color={Math.random() > 0.5 ? "#5f27cd" : "#0abde3"}
            transparent
            opacity={0.5}
          />
        </mesh>
